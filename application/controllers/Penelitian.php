 ...
 *
 */
class penelitian extends Front_Controller
{
	...
	public function view()
	{
		...
		$this->load->model('kegiatan/kegiatan_model');
		$this->load->model('kegiatan/anggota_penelitian_model');
		$this->load->library('users/auth');
		$this->set_current_user();
		$id = $this->uri->segment(3);
		$id_berita = "";
		// berita terbaru
		$this->db->query('update kegiatan set count_view = count_view+1 where id = "'.$id.'"');
		$recordpenelitian = $this->kegiatan_model->find($id);
        ...
        // anggota penelitian 
        $anggotapenelitians = $this->anggota_penelitian_model->find_join($id);
        Template::set('anggotapenelitians', $anggotapenelitians);
        // penelitian lainnya
        $recordpenelitianlainnya = $this->kegiatan_model->find_other($id,$recordpenelitian->tahun);
