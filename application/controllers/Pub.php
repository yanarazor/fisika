 ...
 *
 */
class pub extends Front_Controller
{
	...
	{
		$this->load->model('publikasi/publikasi_model');
		$this->load->library('users/auth');
		$this->set_current_user();
		$years = $this->publikasi_model->getyears();
		Template::set('years', $years);
		
		//$recordpublikasi = $this->publikasi_model->find_all();
		//Template::set('recordpublikasi', $recordpublikasi);
	...
	public function view()
	{
		...
		$this->load->model('publikasi/publikasi_model');
		$this->load->model('publikasi/penulis_publikasi_model');
		$this->load->library('users/auth');
		$this->set_current_user();
		$id = $this->uri->segment(3);
		$id_berita = "";
		// berita terbaru
		$this->db->query('update publikasi set count_view = count_view+1 where id = "'.$id.'"');
		$recordpenelitian = $this->publikasi_model->find($id);
        ...
        // anggota penelitian 
        $anggotapenelitians = $this->penulis_publikasi_model->find_join($id);
        Template::set('anggotapenelitians', $anggotapenelitians);
        // penelitian lainnya
        $recordpenelitianlainnya = $this->publikasi_model->find_other($id,$recordpenelitian->tahun);
