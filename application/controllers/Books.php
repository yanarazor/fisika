...
class books extends Front_Controller
{
	...
	public function index()
	{
		...
		$years = $this->buku_model->getyears();
	...
	public function view()
	{
		...
		$this->load->model('buku/buku_model');
		$this->load->model('buku/penulis_model');
		$this->load->library('users/auth');
		$this->set_current_user();
		$id = $this->uri->segment(3);
		$id_berita = "";
		// berita terbaru
		$this->db->query('update buku set count_view = count_view+1 where id = "'.$id.'"');
		$recordpenelitian = $this->buku_model->find($id);
        ...
        // anggota penelitian 
        $anggotapenelitians = $this->penulis_model->find_join($id);
        Template::set('anggotapenelitians', $anggotapenelitians);
        // penelitian lainnya
        $recordpenelitianlainnya = $this->buku_model->find_other($id,$recordpenelitian->tahun);
