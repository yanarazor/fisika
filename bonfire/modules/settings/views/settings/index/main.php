...
<fieldset>
    ...
    <div class="control-group<?php echo form_error('languages') ? $errorClass : ''; ?>">
     ...
     <div class="control-group<?php echo form_error('languages') ? $errorClass : ''; ?>">
     	<label class="control-label" for="languages">Slide 1</label>
        <div class="controls">
            <div class="area-pop-up-upload dropzone well dz-clickable"></div>
        </div>
    </div>
    <div class="control-group<?php echo form_error('slide2') ? $errorClass : ''; ?>">
     	<label class="control-label" for="languages">Slide 2</label>
        <div class="controls">
            <div class="slide2 dropzone well dz-clickable"></div>
        </div>
    </div>
</fieldset>
<script type="text/javascript">
    ...
    var slide1 = new Dropzone(".area-pop-up-upload",{
    	 autoProcessQueue: true,
		 url: "<?php echo base_url() ?>admin/content/galleri/uploadberkas",
		 maxFilesize: 300,
		 parallelUploads : 1,
		 method:"post",
		 //acceptedFiles:"image/.*",
		 paramName:"userfile",
		 dictDefaultMessage:"<img src='<?php echo base_url(); ?>assets/images/dropico.png' width='50px'/><br>Drop image disini atau klik area ini untuk browse file",
		 dictInvalidFileType:"Type file ini tidak dizinkan",
		 addRemoveLinks:true,
		});
		 slide1.on("success",function(a,b){
		 	$(".dz-success").remove();
           	swal(b, "Warning");
          });
	slide1.on("sending",function(a,b,c){
		a.token=Math.random();
	...
	slide1.on("success",function(a,b){
		var data_n=JSON.parse(b);
	  	$(".just-upload-field").val(data_n.namafile).select();
	});
</script> 