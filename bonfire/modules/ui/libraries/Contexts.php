...
class Contexts
{
		...
		*/
		foreach ($contexts as $context)
		{
			if ( has_permission('Site.'. ucfirst($context) .'.View') == true || permission_exists('Site.'. ucfirst($context) .'.View') == false)
			{
				...
				$nav .= "<i class='menu-icon fa fa-desktop'></i>&nbsp; ";
    ...
    public static function context_navacc($context = null, $class = 'dropdown-menu', $ignore_ul = false)
    {
        // Get a list of modules with a controller matching $context ('content',
        // 'settings', 'reports', or 'developer').
        foreach (Modules::list_modules() as $module) {
        ...
        // Add any sub-menus and reset the $actions array for the next pass.
        $menu = self::build_sub_menuacc($context, $ignore_ul);
        self::$actions = array();
    ...
    public static function build_sub_menuacc($context, $ignore_ul=FALSE,$menuaktif="")
	{
		...
		foreach (self::$menu as $topic_name => $topic)
		{
			...
			if (count($topic) > 1)
			{
				$list .= '<li class="no-link parent-menu"><a href="#" class="no-link parent-menu">'. self::$ci->lang->line(str_replace('lang:', '', $topic_name)) .'</a>';
				$list .= '<ul>';
				...
				foreach ($topic as $module => $vals)
				{
					...
					{
						$list .= self::build_item($module, $vals['title'], self::$ci->lang->line(str_replace('lang:', '', $vals['display_name'])), $context, $vals['menu_view']);
					}
				}
