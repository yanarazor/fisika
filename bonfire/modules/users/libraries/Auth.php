...
class Auth
{
    ...
    public function login($login, $password, $remember = false)
    {
        ...
        $this->setupSession(
            ...
            $user->eselon_4,$user->eselon_3,$user->unit_kerja
        );
	...
	{ 
		...
		return $this->user()->eselon_4;

	}
	public function get_eselon_3()
	{ 
		if ( ! $this->is_logged_in())
		{
			return FALSE;
    ...
    private function autologin()
    {
        ...
        $this->ci->load->model('users/user_model');
        $user = $this->ci->user_model->select(array('id', 'username', 'email', 'password_hash', 'users.role_id', 'users.nip'))
                                     ->find($user_id);
