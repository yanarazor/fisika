...
?>
    ...
    <fieldset>
        ...
        <div class="control-group">
            <div class="controls">
                <label for="<?php echo $field; ?>">
                    <input type="checkbox" class="minimal" name="<?php echo $field; ?>" id="<?php echo $field; ?>" value="1" />
        ...
        <div class="control-group">
            <div class="controls">
                <label for="restore">
                    <input type="checkbox" class="minimal" name="restore" id="restore" value="1" />
...
<script>
	$("#Unit_Kerja").change(function(){
		var valUnit_Kerja = $("#Unit_Kerja").val();
		$("#eselon_3").empty().append("<option>loading...</option>"); //show loading...
		 
		var json_url = "<?php echo base_url(); ?>admin/masters/eselontiga/getdatselect?unitkerja=" + encodeURIComponent(valUnit_Kerja);
		$.getJSON(json_url,function(data){
			$("#eselon_3").empty(); 
			if(data==""){
				$("#eselon_3").append("<option value=\"\"> Pilih </option>");
			}
			else{
				$("#eselon_3").append("<option value=\"\">-- Pilih --</option>");
				for(i=0; i<data.id.length; i++){
					$("#eselon_3").append("<option value=\"" + data.id[i]  + "\">" + data.nama_bagian[i] +"</option>");
				}
			}
			
		});
		
		$("#eselon_4").empty().append("<option>loading...</option>"); //show loading...
		 
		var json_url = "<?php echo base_url(); ?>admin/masters/eselonempat/getdatselect?unitkerja=" + encodeURIComponent(valUnit_Kerja);
		$.getJSON(json_url,function(data){
			$("#eselon_4").empty(); 
			if(data==""){
				$("#eselon_4").append("<option value=\"\"> Pilih </option>");
			}
			else{
				$("#eselon_4").append("<option value=\"\">-- Pilih --</option>");
				for(i=0; i<data.id.length; i++){
					$("#eselon_4").append("<option value=\"" + data.id[i]  + "\">" + data.nama_subbag[i] +"</option>");
				}
			}
